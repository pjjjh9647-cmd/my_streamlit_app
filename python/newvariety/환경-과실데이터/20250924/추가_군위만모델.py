...
# 사용자 입력
# =========================
ENV_DAILY_PATH    = r"C:\Users\User\Desktop\mba\환경데이터\기상데이터_통합.xlsx"   # 일별 기상
FRUIT_TARGET_PATH = r"C:\Users\User\Desktop\mba\과실데이터\과실데이터_통합.xlsx" # 과실 지표
...
# 엑셀 시트명에 금지문자 있을 경우 안전하게 바꿔주는 함수
INVALID_SHEET_CHARS = '[]:*?/\\'
def sanitize_sheetname(name: str, maxlen: int = 31) -> str:
    for ch in INVALID_SHEET_CHARS:
        name = name.replace(ch, "_")
    return name[:maxlen]

# 월 접미사 mMM을 읽기 위한 정규식 예: ..._m07
...
def filter_columns_by_month(df: pd.DataFrame, allowed_months: list[int]) -> pd.DataFrame:
    """월별 피처 중 allowed_months에 해당하는 열만 남긴다. 키/메타 열은 유지."""
...
def load_inputs():
    ...
    # 지역 필터 적용: 군위만 남김
    env = env[env[COL_REGION].astype(str).str.strip() == REGION_FILTER].copy()
    fruit = fruit[fruit[COL_REGION].astype(str).str.strip() == REGION_FILTER].copy()
    if len(env) == 0 or len(fruit) == 0:
        raise ValueError("군위 지역으로 필터 후 데이터가 비었습니다. 파일 내 지역명을 확인하세요.")
...
# 타깃별 단일 파이프라인
# =========================
def run_for_target(env_daily, fruit_sub, env_cols, target_col, cultivar_name=None):
    # 1) 월별 피처 생성
...
def main():
    ...
    outfile = OUTDIR / f"통합회귀_환경만_MULTI_FIXED_군위_{ts}.xlsx"

    # 품종 그룹핑 여부
    if "품종" in fruit.columns and GROUP_BY_CULTIVAR:
    ...
    with safe_excel_writer(outfile) as writer:
        for cv in cultivars:
            ...
            for tgt in targets:
                try:
                    model, eq, merged, corr_s, summary, features, lasso_coef = run_for_target(
                        env, fsub, env_cols, tgt, cultivar_name=cv
                    )
                    # 시트 접두에 월 범위 표시
                    if cv is not None and cv in CULTIVAR_MONTH_WINDOWS:
                        mm = CULTIVAR_MONTH_WINDOWS[cv]
                        mm_str = f"(m{min(mm):02d}-m{max(mm):02d})"
                    else:
                        mm_str = "(m01-m12)"

                    sheet_prefix = f"{tgt}{cv_tag}{mm_str}"
