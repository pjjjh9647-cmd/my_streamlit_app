...
# --- 탭5, 6에서 필요한 더미 함수/변수 정의 (실제 구현 필요) ---
def gw_load_merged_dataset():
    # (merged_all, fruit_agg, env_mwide, bio_y, meta)
    import pandas as pd
    # 빈 데이터프레임 및 메타 튜플 반환
...
def gw_load_selected_vars(cv):
    import pandas as pd
    # target, vars_list 컬럼이 있는 더미 데이터프레임
...
def gw_fit_ols(g, tgt, xcols):
    ...
    return DummyModel(), pd.DataFrame({"year": [2024], tgt: [0.0]}), pd.Series([0.0]), {"R2": 0.0, "RMSE": 0.0}

GW_MONTH_RANGE = {"홍로": list(range(4,9)), "후지": list(range(4,11))}

def gw_plot_coefficients(model, title=None):
    import matplotlib.pyplot as plt
    fig, ax = plt.subplots(); ax.set_title(title or "Coefficients"); plt.close(fig)
    st.pyplot(fig)

def gw_plot_observed_vs_pred(sub, yhat, tgt, title=None):
    import matplotlib.pyplot as plt
    fig, ax = plt.subplots(); ax.set_title(title or "Observed vs Predicted"); plt.close(fig)
    st.pyplot(fig)

def gw_plot_metric_bar(metric, value):
    import matplotlib.pyplot as plt
    fig, ax = plt.subplots(); ax.bar([metric], [value]); plt.close(fig)
    st.pyplot(fig)
# 첫번째 탭: 분석결과 (tabl~6)
...
# 파일명: 기상자동불러오기3.py
    ...
    def apply_equation_row(row: pd.Series, eq_str: str) -> float:
        ...
        import re
        for c in cols:
            # 변수명 앞뒤가 숫자/영문/한글이 아닌 경우만 치환 (부분치환 방지)
            expr = re.sub(rf'(?<![\w가-힣]){re.escape(c)}(?![\w가-힣])', f"row[{repr(c)}]", expr)
...
with tab5:
    ...
    if not cultivars_avail:
        st.warning("분석 결과 폴더(_ANALYSIS_OUT/_BY_CULTIVAR)가 비어 있습니다. 먼저 분석 스크립트를 실행해 주세요.")
        st.stop()
    ...
    for cv in cultivars_avail:
        ...
        g = merged_all[(merged_all[cultivar_col] == cv) & (merged_all["year"].isin(common_years))].copy()

        for _, r in sel_df.iterrows():
            tgt = r["target"]
            xcols = [x for x in r["vars_list"] if x in g.columns and (x.startswith("BIO") or x.endswith(month_suffix))]
            ...
            rows.append({
                ...
                "표본수": len(sub), "R²": mets["R2"], "RMSE": mets["RMSE"],
            })

    if not rows:
        st.warning("성능 요약을 만들 데이터가 없습니다.")
        st.stop()
    ...
    g = merged_all[(merged_all[cultivar_col] == cv_sel) & (merged_all["year"].isin(common_years))].copy()
    ...
    if model is None:
        ...
        with colC:
            gw_plot_metric_bar("R²", mets["R2"])
        with colD:
            gw_plot_metric_bar("RMSE", mets["RMSE"])
...
with tab6:
    ...
    # 사용 변수들(회귀식에 등장하는 한글 변수들 위주)
    num_cols_env = [c for c in ["평균기온","최고기온","최저기온","습도","강우량","일사량","평균풍속","최대풍속","결로시간"]
                    if c in env_raw.columns]
    for c in num_cols_env:
        env_raw[c] = pd.to_numeric(env_raw[c], errors="coerce")

    # 월별 집계: mean & sum
    mean_map = {c:"mean" for c in num_cols_env}
    sum_map  = {c:"sum"  for c in num_cols_env}
    env_month = env_raw.dropna(subset=["연도","월"]).groupby(["연도","월"], as_index=False).agg({**mean_map, **sum_map})
    # mean/sum 컬럼명 분리 정리
    for c in num_cols_env:
